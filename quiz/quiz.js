...
// main object
Quiz = {
    QUIZ: "<div id='questions'/><button id='submit' class='btnClass' /><div id='d1' aria-live='assertive'><p id='score'/></div>",
    ...
    CORRECT: "<span class='correct'>Ghjustu.</span>",
    INCORRECT: "<span class='incorrect'>Falsu.</span><br/><span>A risposta hè: </span><span class='answer'></span>",
    FLASHCARD: "<div id='flashcard'><p class='results'/></div><button id='okButton'>Ok</button>",
    RESET: "<button id='reset' class='btnClass'>Ricumincià</button>",
    ...
    initFromXmlDoc: function(xmldoc) {
        ...
        if (this.mode == null || this.mode == 'undefined' || this.mode == 'show-all') {
            this.mode = 'show-all';
            this.button.text("Valutà");
			this.button.text.fontsize="24px";
    ...
    createList: function(listXml, type) {
        ...
        listXml.children("choice").each(function() {
			...
			li.find("label input").attr("class", "increase");
